...
func doEnableLogging(rc *cmdutils.ResourceCmd) error {
	...
	if updateRequired {
		cmdutils.LogIntendedAction(rc.Plan, "enable CloudWatch logging for cluster %q in %q (enable types: %s & disable type: %s)",
			meta.Name, meta.Region,
			strings.Join(shouldEnable.List(), ", "),
			strings.Join(shouldDisable.List(), ", "),
		)
		if !rc.Plan {
			if err := ctl.UpdateClusterConfigForLogging(cfg); err != nil {
				return err
			}
			logger.Success("updated CloudWatch logging")
		}
	}
